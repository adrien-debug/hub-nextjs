            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-start mb-16">
                  <span className={`font-mono text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>ID.{project.id}</span>
                  {project.id === "01" && (
                    <div className="flex items-center gap-2">
                      <span className={`font-mono text-[10px] uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>NODE // CORE ACTIVE</span>
                      <span className="w-2 h-2 bg-accent rounded-full cyan-glow animate-pulse"></span>
                    </div>
                  )}
                  {project.id !== "01" && (
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <span className={`font-mono text-[10px] uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>ACTIVE</span>
                ...
                <div>
                  <div className={`text-[10px] font-bold uppercase tracking-widest mb-4 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{project.cat}</div>
                  ...
                  <h3 className="text-4xl md:text-5xl font-black tracking-tighter uppercase mb-6">
                    {project.id === "01" ? (
                      <>Onyx <span className="text-accent/90">Pay</span></>
                    ) : (
                      project.name
                    )}
                  </h3>
